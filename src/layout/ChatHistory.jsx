            ...
            <div className={"w-full h-full flex flex-col justify-center items-center"}>
                    ...
                    <h2 className={"w-full text-center hover:bg-gray-200 hover:cursor-pointer mb-1"}>채팅 기록이 없습니다.</h2>}
                {
                    chatBotRooms.map((chatBotRoom, index)=>{
                        return(
                            <div key={index}
                                 className={"overflow-hidden flex px-2 justify-center items-center hover:cursor-pointer hover:bg-gray-200"}>
                                <h2
                                    onClick={() => navigate(`/chatbot/${chatBotRoom.uuid}`)}
                                    className={"w-full h-6 mb-1 text-[#303e4f]"}>
                                    {chatBotRoom.title}
                                </h2>
